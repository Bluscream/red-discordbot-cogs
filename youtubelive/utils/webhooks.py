...
log = logging.getLogger("red.blu.youtubelive.utils.webhooks")

async def ensure_webhook(channel: discord.TextChannel, name: str = "YouTube Mirror") -> Optional[str]:
    """
    ...
    try:
        ...
        new_wh = await channel.create_webhook(name=name, reason="YouTube Live mirror initialization.")
        return new_wh.url
...
async def delete_webhook_by_url(url: str, reason: Optional[str] = None):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            await webhook.delete(reason=reason or "YouTube monitor stopped.")
